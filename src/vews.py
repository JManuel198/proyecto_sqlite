...
class Star():
    ...
    def crear_nuevo_ventana(self):
        ...
        self.btn_accion = tk.Button(self.ventana_nuevo, text="Guardar", bg="#40a02b", fg="white", 
                                    command=self.guardar_registro_bd)
        self.btn_accion.grid(row=20, column=1, pady=10)
        ...
        if registro:
            ...
            for child in self.ventana_nuevo.winfo_children():
                if isinstance(child, tk.Button) and child["text"] == "Guardar":
                    self.btn_accion.config(text="Actualizar datos", command=self.actualizar_registro_bd)
    ...
    def actualizar_registro_bd(self):
        ...
        for valor, nombre_campo in campos_obligatorios:
            if not valor:
                messagebox.showwarning("Atención", f"No puede dejar el campo '{nombre_campo}' vacío.", parent=self.ventana_nuevo)
                return

        try:
            # Desbloqueamos temporalmente para leer la cédula (PK)
    ...
    def guardar_registro_bd(self):
        ...
        # 2. Restricciones: Campos NO pueden estar vacíos
        campos_obligatorios = [
            (cedula, "Cédula"),
            (nombres, "Nombres"),
            (apellidos, "Apellidos"),
        ...
        # 3. Validación numérica de cédula
        ...
        try:
            ...
            datos = (
                cedula, nombres, apellidos, self.ent_fecha_nacimiento.get(),
                direccion, self.ent_correo.get(), self.ent_telefono.get(),
                self.ent_carnet_patria.get(), self.ent_titulo.get(),
                self.ent_fecha_ingreso.get(), comuna, self.ent_cargo.get(),
            ...
            messagebox.showerror("Error Crítico", f"No se pudo guardar: {e}", parent=self.ventana_nuevo)
